...
export type CommentTargetType = "DESIGN" | "PROBLEM" | "JOB" | "EDITORIAL";
...
interface Props {
  /** Which content type this comment thread belongs to */
  targetType: CommentTargetType;
  /** The id of the design/problem/job/article being commented on */
  targetId: string;
  comments: Comment[];
  currentUserId?: string;
  /** Optional copy override, e.g. "Discussion" instead of "Comments" */
  heading?: string;
  /** Optional empty-state copy override */
  emptyText?: string;
}

// Maps targetType -> the field name the API expects (designId / problemId / jobId / articleId)
...
function CommentItem({ comment, onReply }: {
  ...
  const authorName = comment.author.designerProfile?.alias ?? comment.author.name ?? "User";
                ...
                <div>
                  <div className="flex items-baseline gap-2">
                    <span className="text-xs font-medium">
                      {reply.author.designerProfile?.alias ?? reply.author.name ?? "User"}
                    </span>
...
export function CommentSection({
  targetType, targetId, comments: initial, heading, emptyText,
}: Props) {
  ...
  const idField = TARGET_FIELD[targetType];
  ...
  async function postComment() {
    ...
    const res = await fetch("/api/comments", {
      ...
      body: JSON.stringify({ [idField]: targetId, content: newComment, targetType }),
    });
    setPosting(false);
    if (res.ok) {
  ...
  async function postReply(parentId: string, content: string) {
    ...
    const res = await fetch("/api/comments", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ [idField]: targetId, content, targetType, parentId }),
    });
    if (res.ok) {
      const d = await res.json();
  ...
  return (
    <div>
      <h2 className="font-medium mb-4">{heading ?? "Comments"} ({comments.length})</h2>
      ...
      {/* Comments list */}
      {comments.length === 0 ? (
        <p className="text-sm text-muted-foreground">{emptyText ?? "No comments yet. Be the first!"}</p>
      ) : (
